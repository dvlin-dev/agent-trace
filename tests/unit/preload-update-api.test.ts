...
describe("preload update api", () => {
  ...
  it("exposes multi-provider bridge methods and subscriptions", async () => {
    ...
    expect(invokeMock).toHaveBeenNthCalledWith(2, IPC.GET_PROFILES);
    expect(invokeMock).toHaveBeenNthCalledWith(3, IPC.SAVE_PROFILES, []);
    expect(invokeMock).toHaveBeenNthCalledWith(
      4,
      IPC.START_PROFILE,
      "anthropic-dev",
    ...
    expect(invokeMock).toHaveBeenNthCalledWith(6, IPC.GET_PROFILE_STATUSES);
    expect(invokeMock).toHaveBeenNthCalledWith(7, IPC.LIST_SESSIONS, {
    ...
    expect(invokeMock).toHaveBeenNthCalledWith(10, IPC.CLEAR_HISTORY);
